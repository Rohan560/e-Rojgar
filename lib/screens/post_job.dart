...
class _PostJobState extends State<PostJob> {
  ...
  void fillDefaults(BuildContext context, User user) {
    companyNameController.text = user.name;
    emailController.text = user.email;
    phoneController.text = user.phoneNumber ?? "Enter Contact Number";
    websiteController.text = user.website ?? "Enter Website ";
  }

  @override
  Widget build(BuildContext context) {
    return Consumer<UserController>(builder: (context, userController, child) {
      fillDefaults(context, userController.user!);
      return Form(
        key: _formKey,
        child: Scaffold(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (userController.user!.canPostJobFreely() &&
                      userController.role != UserRole.admin)
                    Container(
                      ...
                      child: Padding(
                        padding: const EdgeInsets.all(8.0),
                        child: Text(
                          "You can post your first $kFreeJobPosts jobs for free.",
                          textAlign: TextAlign.center,
                        ),
                      ),
                    )
                  else if (!_paidWithKhalti)
                    Container(
                      width: double.infinity,
                      color: Colors.green.withOpacity(0.3),
                      child: Padding(
                        ...
                        child: Column(
                          children: [
                            Text(
                              "Pay NRs. 20 with Khalti to post your job.",
                              textAlign: TextAlign.center,
                            ),
                            SizedBox(
                              height: 15,
                            ...
                            ElevatedButton(
                              onPressed: () {
                                KhaltiScope.of(context).pay(
                                    config: PaymentConfig(
                                        ...
                                        productName: "Job post in ERojgar"),
                                    onSuccess: (model) {
                                      print("Payment Succeeded.");
                                      print(model.toString());
                                      setState(() {
                                        _paidWithKhalti = true;
                                      });
                                    },
                                    onFailure: (model) {
                              ...
                              child: Text("Pay With Khalti (NRs. 20)"),
                            ),
                          ],
                        ),
                      ),
          ...
          bottomNavigationBar: BottomAppBar(
              child: Container(
                ...
                child: Padding(
                          padding: const EdgeInsets.symmetric(horizontal: kUnitPadding * 2),
                          child: ElevatedButton(
                child: Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: Text("Post"),
                ),
                onPressed: (!userController.user!.canPostJobFreely() &&
                            _paidWithKhalti) ||
                        userController.user!.canPostJobFreely()
                    ? () async {
                        if (_formKey.currentState!.validate()) {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                              content: Text("Some field is not valid.")));
                        }
                      }
                    : null,
                          ),
